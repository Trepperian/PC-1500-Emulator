...
extern "C-unwind" fn change_model_dmg() {
    send_model_event(ceres_core::Model::Dmg);
}

extern "C-unwind" fn change_model_mgb() {
    send_model_event(ceres_core::Model::Mgb);
}

extern "C-unwind" fn change_model_cgb() {
    send_model_event(ceres_core::Model::Cgb);
}
...
fn send_speed_event(speed: u32) {
    if let Some(proxy) = EVENT_PROXY.get() {
        proxy.send_event(CeresEvent::ChangeSpeed(speed as f32)).ok();
    }
}

fn send_model_event(model: ceres_core::Model) {
    if let Some(proxy) = EVENT_PROXY.get() {
