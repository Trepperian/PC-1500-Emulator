...
impl Keyboard {
    ...
    pub fn press(&mut self, key: Key, ints: &mut Interrupts) {
        let key_code = key as u8;
        let bit_pos = key_code & 0x3F; // Use lower 6 bits for bit position (0-63)
        
        self.pressed_keys |= 1u64 << bit_pos;
        
        // Request interrupt for keyboard input (using existing interrupt method)
        ints.request_p1(); // TODO: Replace with proper keyboard interrupt when available
    }
    ...
    pub fn read_input_pins_in0_in7(&self) -> u8 {
        ...
        for row in 0..=0xF {
            for col in 0..=0x5 { // PC-1500 has 6 columns (0-5)
    ...
    pub fn read_column_state(&self, column: u8) -> u8 {
        ...
        if column <= 5 { // PC-1500 has columns 0-5
            for row in 0..=0xF {
